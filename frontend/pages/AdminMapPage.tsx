...
export const AdminMapPage: React.FC = () => {
  ...
  const [evTopics, setEvTopics] = useState<string[]>([]);
  const [evContent, setEvContent] = useState('');
  const [evAudiences, setEvAudiences] = useState<string[]>([]);
  const [evPlace, setEvPlace] = useState('');
  const [evSaving, setEvSaving] = useState(false);
  const [evMsg, setEvMsg] = useState<string | null>(null);
  const TOPIC_OPTIONS = ['Giáo dục giới tính','Giáo dục sức khỏe sinh sản','Giáo dục sức khỏe tâm lý'];
  const AUDIENCE_OPTIONS = ['Học sinh THCS','Học sinh THPT','Phụ Huynh'];
  const [topicsOpen, setTopicsOpen] = useState(false);
  const [audiencesOpen, setAudiencesOpen] = useState(false);
  const [evPlaceSug, setEvPlaceSug] = useState<any[]>([]);
  const [evPlaceOpen, setEvPlaceOpen] = useState(false);
  const evPlaceSugTimerRef = useRef<any>(null);
  const evPlaceBoxRef = useRef<any>(null);
  const saveEvent = async () => {
    if (!evName.trim() || !evStart.trim() || !evEnd.trim() || !evPlace.trim()) { setEvMsg('Vui lòng nhập đầy đủ Tên, Thời gian, Địa điểm'); return; }
    setEvSaving(true);
    try {
      ...
      try {
        ...
        params.set('q', evPlace.trim());
        const r = await fetch(`https://nominatim.openstreetmap.org/search?${params.toString()}`, { headers: { 'accept-language': 'vi' } });
        const d = await r.json();
        if (Array.isArray(d) && d[0]?.lat && d[0]?.lon) {
          const la = parseFloat(d[0].lat); const lo = parseFloat(d[0].lon);
          if (!Number.isNaN(la) && !Number.isNaN(lo)) { lat = la; lon = lo; }
        }
      } catch {}
      const evObj: any = { name: evName.trim(), start: evStart.trim(), end: evEnd.trim(), topics: evTopics.slice(), content: evContent.trim(), audiences: evAudiences.slice(), place: evPlace.trim(), ts: Date.now() };
      if (lat !== null && lon !== null) { evObj.lat = lat; evObj.lon = lon; }
      arr.push(evObj);
  ...
  const fetchEvPlaceSuggestions = async () => {
    const query = evPlace.trim();
    if (query.length < 3) { setEvPlaceSug([]); setEvPlaceOpen(false); return; }
    const params = new URLSearchParams();
    params.set('format', 'jsonv2');
    params.set('addressdetails', '1');
    ...
    if (Array.isArray(data)) { setEvPlaceSug(data); setEvPlaceOpen(true); } else { setEvPlaceSug([]); setEvPlaceOpen(false); }
  };
  useEffect(() => {
    if (evPlaceSugTimerRef.current) clearTimeout(evPlaceSugTimerRef.current);
    evPlaceSugTimerRef.current = setTimeout(fetchEvPlaceSuggestions, 350);
    return () => { if (evPlaceSugTimerRef.current) clearTimeout(evPlaceSugTimerRef.current); };
  }, [evPlace]);
  useEffect(() => {
    const onDocMouseDown = (e: any) => { if (evPlaceBoxRef.current && !evPlaceBoxRef.current.contains(e.target)) setEvPlaceOpen(false); };
  ...
  const search = async () => {
    ...
    try {
      ...
      const n = (s: string) => s.toLowerCase().normalize('NFD').replace(/[ -\u036f]/g, '');
        ...
        <div className="flex flex-col lg:flex-row gap-4 h-[calc(100vh-250px)]">
          <div className="w-full lg:w-2/3 xl:w-3/4 bg-slate-100 dark:bg-slate-900 rounded-xl relative overflow-hidden">
            ...
            >
              ...
              {eventMarks.map((m, idx) => (
                <CircleMarker key={`${m.lat},${m.lon},${idx}`} center={[m.lat, m.lon]} radius={8} pathOptions={{ color: '#f59e0b', fillColor: '#f59e0b', fillOpacity: 0.8 }}>
                  <Tooltip direction="top" opacity={1} permanent={false}>
                    <div className="text-xs">
                      <div className="font-bold">{m.name}</div>
                      <div>{m.start}{m.end ? ` — ${m.end}` : ''}</div>
                      <div className="truncate max-w-[180px]">{m.place}</div>
                    </div>
                  </Tooltip>
                  <Popup>
          ...
          <div className="w-full lg:w-1/3 xl:w-1/4 flex flex-col space-y-4">
            <div className="bg-slate-100 dark:bg-slate-900 rounded-lg p-4">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-3">Tạo Sự Kiện</h3>
              <div className="space-y-3">
                <div>
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Tên sự kiện</label>
                  <input value={evName} onChange={(e)=>setEvName(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-md bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white" />
                </div>
                <div>
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Thời gian</label>
                  <div className="mt-1 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400">Bắt đầu</span>
                      <input type="datetime-local" value={evStart} onChange={(e)=>setEvStart(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-md bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white" />
                    </div>
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400">Kết thúc</span>
                      <input type="datetime-local" value={evEnd} onChange={(e)=>setEvEnd(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-md bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white" />
                    </div>
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Chủ Đề</label>
                  <div className="mt-1 relative">
                    <button type="button" onClick={()=>setTopicsOpen(v=>!v)} className="w-full px-3 py-2 rounded-md bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-left flex items-center justify-between text-slate-900 dark:text-white">
                      <span className="truncate">{evTopics.length ? evTopics.join(', ') : 'Chọn chủ đề'}</span>
                      <svg className={`w-4 h-4 transition-transform ${topicsOpen ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M6 9l6 6 6-6" strokeWidth="2"/></svg>
                    </button>
                    {topicsOpen && (
                      <div className="absolute z-10 mt-2 w-full rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-lg">
                        <ul className="max-h-48 overflow-auto">
                          {TOPIC_OPTIONS.map(opt => (
                            <li key={opt} className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-700">
                              <input type="checkbox" checked={evTopics.includes(opt)} onChange={(e)=>{
                                const checked = e.target.checked;
                                setEvTopics(prev => checked ? [...prev, opt] : prev.filter(x=>x!==opt));
                              }} />
                ...
                <div>
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Nội Dung Chương Trình</label>
                  <textarea rows={4} value={evContent} onChange={(e)=>setEvContent(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-md bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white" />
                </div>
                <div>
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Phù hợp với các đối tượng</label>
                  <div className="mt-1 relative">
                    <button type="button" onClick={()=>setAudiencesOpen(v=>!v)} className="w-full px-3 py-2 rounded-md bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-left flex items-center justify-between text-slate-900 dark:text-white">
                      <span className="truncate">{evAudiences.length ? evAudiences.join(', ') : 'Chọn đối tượng'}</span>
                      <svg className={`w-4 h-4 transition-transform ${audiencesOpen ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M6 9l6 6 6-6" strokeWidth="2"/></svg>
                    </button>
                    {audiencesOpen && (
                      <div className="absolute z-10 mt-2 w-full rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-lg">
                        <ul className="max-h-48 overflow-auto">
                          {AUDIENCE_OPTIONS.map(opt => (
                            <li key={opt} className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-700">
                              <input type="checkbox" checked={evAudiences.includes(opt)} onChange={(e)=>{
                                const checked = e.target.checked;
                                setEvAudiences(prev => checked ? [...prev, opt] : prev.filter(x=>x!==opt));
                              }} />
                              <span className="text-sm text-slate-900 dark:text-white">{opt}</span>
                            </li>
                          ))}
                ...
                <div>
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Địa điểm</label>
                  <div className="mt-1 relative" ref={evPlaceBoxRef}>
                    <input value={evPlace} onChange={(e)=>setEvPlace(e.target.value)} onFocus={()=>{ if (evPlaceSug.length>0) setEvPlaceOpen(true); }} className="w-full px-3 py-2 rounded-md bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white" />
                    {evPlaceOpen && evPlaceSug.length > 0 && (
                      <div className="absolute z-10 mt-2 w-full rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-lg">
                        <ul className="max-h-48 overflow-auto">
                          {evPlaceSug.map((item:any) => (
                            <li key={`${item.place_id}`}>
                              <button type="button" onClick={()=>selectEvPlaceSuggestion(item)} className="w-full text-left px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-700">
                                <span className="text-sm text-slate-900 dark:text-white truncate">{item.display_name}</span>
                              </button>
                            </li>
                          ))}
                        </ul>
                ...
                {evMsg ? (<p className="text-sm text-slate-600 dark:text-slate-300">{evMsg}</p>) : null}
                <div className="flex justify-end">
                  <button onClick={saveEvent} disabled={evSaving} className="px-4 py-2 rounded-md bg-cyan-500 text-white hover:bg-cyan-600 font-bold">Lưu</button>
                </div>
              </div>
            ...
            <div className="flex-grow p-4 bg-slate-100 dark:bg-slate-900 rounded-lg min-h-0">
              {false ? (
                ...
                <div className="h-full overflow-y-auto custom-scroll rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 border-t-0 shadow-inner">
                  ...
                  {userPos && nearOnly ? (
                    nearby.length > 0 ? (
                      ...
                      nearLoading ? <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-6">Đang tìm kết quả gần bạn…</p> : <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-6">Không có kết quả trong {RADIUS_KM} km quanh vị trí của bạn.</p>
                    )
                  ) : (
                    (() => {
