...
export const ScenarioPage: React.FC<{ userRole: UserRole }> = ({ userRole }) => {
  ...
  const isWrongSelection = (() => {
    if (selectedOption == null || !scenario) return false;
    const fb = scenario.options[selectedOption]?.feedback ?? '';
    return isIncorrectFeedback(fb);
  })();
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-2xl shadow-xl shadow-slate-900/5 dark:shadow-[0_0_18px_rgba(15,23,42,0.35)] border border-slate-200 dark:border-slate-700 min-h-[400px] flex flex-col justify-center items-center">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.27.735A5.002 5.002 0 0014 13h-3a1 1 0 110-2h5a1 1 0 011 1v4a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.735-1.27z" clipRule="evenodd" /></svg>
          <span>{isWrongSelection ? 'Thử lại' : 'Tạo tình huống mới'}</span>
        </button>
      </div>
    </div>
  );
